...
const OrderDetailsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      ...
      try {
        setLoading(true);
        const response = await fetch(`https://vitals-iu4r.onrender.com/api/customer/orders/${orderId}/details`, {
          headers: {
  ...
  const handleConfirmDelivery = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://vitals-iu4r.onrender.com/api/customer/orders/${orderId}/status`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
